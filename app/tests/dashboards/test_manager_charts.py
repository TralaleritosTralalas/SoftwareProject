...
class ManagerDashboardChartsTest(BaseTestSuite):
    def setUp(self):
        super().setUp()
    ...
    def test_manager_export_csv_is_valid(self):
        response = self.client.get(reverse('app:manager_dashboard'), {'export': 'csv'})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response['Content-Type'], 'text/csv')

    def test_manager_dashboard_chart_data_presence(self):
